...
課程頁的檢核進度只保存在該瀏覽器的 `localStorage`。前測於開課當下至開課後 30 分鐘內開放；後測於下課前 10 分鐘開放，並合併 5 題滿意度。前、後測分別送至兩份 Google 表單，再寫入同一份講師私人 Google Sheets 的不同分頁。送出內容只有場次、課程、該場隨機匿名代碼、固定選項與時間；不含姓名、Email、單位或自由文字。講師教材與答案另外保存在只授權講師帳號的私人 Google Drive，不會由公開網站連結或載入。
...
- 六場日期與起訖時間正確，前測採 `[上課開始, 上課後 30 分鐘)`、後測採 `[下課前 10 分鐘, 下課時間)`。

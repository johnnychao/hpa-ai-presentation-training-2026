# 公開內容政策

本 repository 與 GitHub Pages 預設為網際網路公開。即使畫面沒有顯示連結，只要檔案曾被 commit 或部署，就不能視為保密。

## 可以公開

- 已核准的課程名稱、梯次、日期、時段、難度、目標、主軸與學員產出。
- 課程流程、NotebookLM 學員操作步驟、學員提示詞、虛構案例、工作模板、品質檢核、評量與一般使用說明。
- 明確標示「教學虛構、非國健署統計、非真實個案、不得對外引用」的學生操作資料包。
- 每場「已開放／尚未開放」狀態。
- 專為本網站生成、沒有真人識別資訊的微縮卡通情境圖。
- 由國民健康署官方網站取得、用於本課程識別的官方 Logo。
- 網站 HTML、CSS、JavaScript、公開 JSON 與維運文件。

## 不得公開

- 姓名、電子郵件、電話、身分證字號、員工編號、IP、帳號或其他個人識別資料。
- 報名表、學員名單、簽到表、回饋原始資料、出席紀錄或資格判定；匿名回覆只可存在講師私人 Google Sheets。
- 簽名文件、報價、預算、契約、內部公文、未公開研究或未核准教材。
- 會議連結、會議密碼、僅限學員的下載網址或任何以隱藏 URL 代替權限控管的內容。
- API key、GitHub token、Google token、密碼、私鑰、`.env` 或系統設定秘密。
- Office 文件、PDF、資料庫、影音原檔與壓縮包；CSV 只允許位於指定學生案例資料夾，且每列明示為教學合成資料。
- 已由需求撤回的額外簡報修改主題。
- 講師提示、講者備註、參考答案、故意植入錯誤清單、修正版標題、建議方案與講師專用提示詞。
- 私人 Google Drive 的檔案 ID、共享網址或任何可推知講師教材位置的連結。

## 講師教材的存放

公開網站不提供講師模式，也不載入講師教材。講師手冊、答案與帶領提示應存放在私人 Google Drive，僅授權講師自己的 Google 帳號；不得以「未在首頁顯示」或「知道網址才看得到」取代帳號權限。

公開 repository 不得反向連結私人教材。若私人教材要引用學員內容，可由私人文件單向連到本網站。

## 開放狀態的界線

`availability.json` 只是公開介面的顯示設定：

- `isOpen: true`：顯示「已開放」入口。
- `isOpen: false`：顯示「尚未開放」。
- `defaultOpen: false`：資料讀取失敗時全部關閉。

這不是登入、授權或存取控制。若教材必須只給特定學員，應使用機關核准的 Google Workspace、Microsoft 365、LMS 或其他具身分驗證的系統。

六個場次內容頁都會隨 GitHub Pages 公開部署；未開放時只是不在首頁提供入口，知道網址者仍可能直接瀏覽。

## 瀏覽器本機進度與匿名評量

課程頁可讓使用者勾選檢核項目。狀態只寫入使用者自己的瀏覽器 `localStorage`：

- 不建立帳號，也不寫入姓名、電子郵件或其他身分資訊。
- 不提供自由文字輸入欄位。
- 檢核進度不傳送到 GitHub、國民健康署或第三方服務。
- 清除瀏覽器網站資料即可移除本機進度。

前後測是另一條明確資料流程：

- 前測與後測使用兩份不同的 Google 表單；後測同次包含 5 題滿意度。
- 只傳送場次、課程、該場隨機匿名代碼、固定選項及送出時間，寫入講師私人 Google Sheets 的不同回覆分頁。
- 回覆資料不設姓名、Email、單位、IP 或自由文字欄位；Google 表單也設定為不收集電子郵件。
- 公開網站只包含題目與選項，不包含正解、解析、能力標籤或計分鍵。
- 作答時段只是瀏覽器介面控制，不是身分驗證或真正的安全存取限制。

任何實作內容都應在 NotebookLM 或機關核准的工作環境完成；不得把個資、未公開公文、敏感案件、內部決策草稿或未授權資料貼入本網站。

## 品牌來源

- 官方網站：<https://www.hpa.gov.tw/home/index.aspx>
- 官方 Logo：<https://www.hpa.gov.tw/NewPageStyle/TW/images/logo/logo.png>
- 官方網站圖示：<https://www.hpa.gov.tw/NewPageStyle/TW/images/favicon.ico>
- 本站使用官方 Logo 與官方網站的綠、青綠、灰色系，並標示「課程專用入口」，不冒充或取代國民健康署官網。

## 上線前檢查

每次 Pages 部署都會執行 `node scripts/validate-site.mjs`，阻擋：

- 不符合三梯六場契約的資料。
- 不符合三階完整課程內容契約的資料。
- 缺少三階各 10 題前測、10 題後測、5 題後測滿意度，或前後測誤用同一 Google 表單。
- 公開評量出現正解、解析、能力標籤或計分欄位。
- 缺少學生操作資料包入口，或公開課程出現講師私密欄位。
- 已移除的講師提示詞公開檔重新出現。
- 已取消內容文字。
- 敏感檔名與高風險副檔名。
- 缺少的圖片、CSS、JavaScript、JSON 或錯誤片段連結。
- 基本 HTML 語意與可及性錯誤。

自動檢查不能取代人工審查。新增公開內容前，仍應由內容負責人確認資料已核准、無個資且適合永久公開。
